...
// 渲染单张卡片（横排：主队 比分 客队，轮次配色）
  function cardHtml(roundKey, m) {
    ...
    " onmouseover="this.style.filter='brightness(1.3)'" onmouseout="this.style.filter=''">
      <div style="padding:0 10px;font-size:16px;line-height:1.4;white-space:nowrap;text-align:center !important;">
      ...
      ${m.time ? `<div style="padding:0 10px;font-size:10px;color:rgba(255,255,255,0.35);line-height:1.3;text-align:center !important;">${esc(m.time)}</div>` : ''}
    </div>`;
  }
  ...
  for (const c of TREE_cols) {
    ...
    html += `<div style="position:absolute;left:${x}px;top:0;width:${TREE_colW}px;height:${TREE_headerH - 6}px;line-height:${TREE_headerH - 6}px;text-align:center;border-bottom:1px solid ${tint(colC, 0.30)};">
  ...
  function TREE_lineY(round, idx) { return TREE_y(round, idx) - TREE_padY - 2; }
    ...
    if (m.round === '3rd') {
      [0, 1].forEach(i => {
        ...
        if (_isPlaceholder(m[side])) {
          // API 可能已预填队伍（如 away_team_id=33），优先用 API 数据
          const apiGame = koGames[m.no];
          const apiTeamId = side === 'a' ? (apiGame && apiGame.home_team_id) : (apiGame && apiGame.away_team_id);
          if (apiTeamId && apiTeamId !== '0' && apiTeamId !== 0) {
            const name = _koTeamName(apiTeamId);
            if (name) { m[side] = name; changed = true; return; }
          }
  ...
  try {
    ...
    apiGames.forEach(ag => {
      ...
      if (local) {
        ...
        // 更新淘汰赛队伍ID（API预填对阵信息）
        if (ag.home_team_id && ag.home_team_id !== '0') local.home_team_id = ag.home_team_id;
        if (ag.away_team_id && ag.away_team_id !== '0') local.away_team_id = ag.away_team_id;
        if (ag.home_team_name_en) local.home_team_name_en = ag.home_team_name_en;
        if (ag.away_team_name_en) local.away_team_name_en = ag.away_team_name_en;
        // 更新点球比分
        if (ag.home_penalty != null && ag.home_penalty !== '') { local.home_penalty = ag.home_penalty; local.away_penalty = ag.away_penalty; }
      }
    });
    // 更新 liveIndicator
